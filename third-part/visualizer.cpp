...
/**
 * this program is copy from https://github.com/YoussefRaafatNasry/bst-ascii-visualization/tree/master/BST%20ASCII%20Visualization
 * @tparam T
 * @param tree
 * @param node_length
 * @param space_length
...
visualizer::visualizer(BinarySearchTree &tree, int node_length /* = -1 */, int space_length /* = -1 */) {
    ...
    queue<Node *> nodes = breadth_first_search();
...
queue<Node *> visualizer::breadth_first_search() {
    queue<Node *> temp, nodes;
    ...
    for (auto i = 0; i < tree_nodes_; i++) {
        ...
        if (current == nullptr) {
            ...
            temp.push(current->leftNode);
            temp.push(current->rightNode);
        }
    }
...
int visualizer::get_tree_height(Node *root) const {
    if (root == nullptr) return 0;
    const int left_height = get_tree_height(root->leftNode);
    const int right_height = get_tree_height(root->rightNode);
    return left_height > right_height ? left_height + 1 : right_height + 1;
}


int visualizer::get_nodes_count(const int level) const {
    return int(pow(2, level));
}


int visualizer::get_subtree_width(const int level) const {
...
void visualizer::visualize() const {
